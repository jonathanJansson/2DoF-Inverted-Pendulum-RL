...
class NonLinOdInvPendulumEnv(gym.Env):
    metadata = {
    ...
    def step(self, u):
        ...
        if done:
            ...
            q = np.array([50, 90, 5, 5])
            r = [0]
            unscaled_x = 1.0 - np.divide(np.power(self.state, 2), np.power(self.state_clamp_high, 2))
            unscaled_u = 1.0 - np.divide(np.power(u, 2), np.power(self.action_clamp_high, 2))
            reward = (np.dot(unscaled_x, q) + np.dot(unscaled_u, r)) * self.dt * 1 / (np.sum(q) + np.sum(r))
    ...
    def reset(self):
        ...
        high = np.array(
            [0.0 * np.pi / 180.0, 0.5 * np.pi / 180.0, 0.0, 0.0])
